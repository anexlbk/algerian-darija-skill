...
6. If the task is business or marketing content (product listings, ads, social captions, customer replies) rather than personal chat or storytelling, check `reference/commercial-copy.md` for the brand-voice register, CTA phrasing, and platform formatting conventions. Note that only parts of that file are grounded in real Algerian marketplace data; it says which parts.
...
Built from patterns in raw social/YouTube-comment Darija text, an AI-translated children's-story corpus (for narrative register), a filtered subset of a crowdsourced Darija dictionary (loanwords and expressions only, the dictionary's own card flags it as noisy/spammy, so slang not cross-checked against other sources was left out), and for commercial-copy.md specifically, a large corpus of real Algerian marketplace property listings (for CTA phrasing and formatting conventions only, not for grammar). Two other candidate datasets considered for this skill turned out to be mislabeled Moroccan Darija rather than Algerian and were excluded entirely rather than partially used. Add new phrases to the relevant reference file as they come up rather than creating new files per source.
